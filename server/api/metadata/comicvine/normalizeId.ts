import {
  COMICVINE_PUBLISHER_RESOURCE,
  COMICVINE_VOLUME_RESOURCE,
} from './constants';
...
export const comicVinePublisherApiId = (id: string | number): string => {
  const normalized = normalizeComicVinePublisherId(String(id));

  return `${COMICVINE_PUBLISHER_RESOURCE}-${normalized}`;
};
